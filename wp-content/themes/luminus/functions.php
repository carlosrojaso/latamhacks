...
function wpeden_meta_box_icons(){
        ...
        <select id="icons" name="wpeden_post_meta[icon]" type="text">
        <?php foreach($icons  as $class => $name){ echo "<option value='{$class}' ".selected($class, $icon).">{$name}</option>"; } ?>
        </select> <br>   
        <label for="icons">Excerpt:</label><br>   
        <textarea class="widefat" cols="" rows="" name="wpeden_post_meta[excerpt]"><?php echo $excerpt; ?></textarea>  
        <?php       
...
function wpeden_save_meta_data($postid, $post){
 ...
 function wpeden_meta_boxes(){ 
    ...
    $meta_boxes = array(
                        'wpeden-icons'=>array('title'=>__('Page Info',"digital-product"),'callback'=>'wpeden_meta_box_icons','position'=>'side','priority'=>'core')
                   );
